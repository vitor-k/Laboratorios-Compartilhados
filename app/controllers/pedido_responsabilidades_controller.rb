class PedidoResponsabilidadesController < ApplicationController
  before_action :set_pedido_responsabilidade, only: [:show, :edit, :update, :destroy]
  ...
  def index_docente
    @pedido_responsabilidades_this = PedidoResponsabilidade.all
    @pedido_responsabilidades_this.find_each do |cada|
      if (cada.id_docente != solicitador_responsabilidade.id)
        @pedido_responsabilidades_this.drop(cada.id)
      end
    end
  end
    ...
    if (@permissao_novo && Laboratorio.find(@pedido_responsabilidade.id_laboratorio).responsavel == nil)
      respond_to do |format|
        if @pedido_responsabilidade.save
          format.html { redirect_to @pedido_responsabilidade, notice: 'Pedido responsabilidade was successfully created.' }
    ...
    if (@permissao_existente)
      respond_to do |format|
        if @pedido_responsabilidade.update(pedido_responsabilidade_params)
          format.html { redirect_to @pedido_responsabilidade, notice: 'Pedido responsabilidade was successfully updated.' }
